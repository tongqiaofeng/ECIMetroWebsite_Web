      ...
      >
        <van-tab title="按系列查看" name="first">
          ...
          <div v-if="productList.length > 0">
            ...
            <!-- <div class="check-btn check-all">查看全部</div> -->
            <van-pagination
              v-model="page"
              :total-items="total"
              :items-per-page="6"
        ...
        <van-tab title="按需求查看" name="second">
          ...
          <div class="grab-tab" v-show="maxReachRangeList.length > 0">
    ...
    <div class="product-grab-workers">
      ...
      >
        ...
        <van-tab title="按需求查看" name="second">
          ...
          <div class="check-title" v-show="maxReachRangeList.length > 0">
            最大挖掘深度
          </div>
...
const checkDetail = (item: any) => {
  ...
  router.push({
    path: "/product/detail",
    query: { name: pageName.value, productId: item.id, type: "2" },
  });
};
...
console.log("页面标题", pageName.value);

const { grabId, grabContent, imgClassifyUrl, imgClassifyPhoneUrl, typeId } =
  await getProductClassify("联盟产品", pageName.value);
